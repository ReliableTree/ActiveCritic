...
def run_experiment(device):
    ...
    transitions, rollouts = sample_expert_transitions(vec_expert.predict, env, 100)
    env.envs[0].reset_count = 0
    device = 'cuda'
    pomdp_rollouts = make_pomdp_rollouts(rollouts, 1000, count_dim=10)
    transitions = rollout.flatten_trajectories(pomdp_rollouts)
