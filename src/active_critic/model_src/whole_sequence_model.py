...
class WholeSequenceModel(nn.Module, ABC):
    ...
    def optimizer_step(self, inputs:th.Tensor, label:th.Tensor, autoregressive:bool, prefix='', mask:th.Tensor=None, attention_mask:th.Tensor=None) -> typing.Dict:
        if mask is not None:
            '''if (mask.shape[0] != result.shape[0]) and (result.shape[0] == 1):
                mask = mask.unsqueeze(0)
            loss = self.loss_fct(result=result[mask], label=label[mask])
            '''
            print('tried to use label mask')
            1/0
        else:
            if autoregressive:
                result = self.forward(inputs=inputs, mask=attention_mask)
                loss = self.loss_fct(result=result[:, :-1], label=label[:, 1:])
            else:
...
class CriticSequenceModel(WholeSequenceModel):
    ...
    def forward(self, inputs: th.Tensor) -> th.Tensor:
        ...
        trans_result = trans_result.reshape([trans_result.shape[0], -1])
